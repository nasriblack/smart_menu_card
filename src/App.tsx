...
function App() {
  ...
  const dummyResponse = {
    suggestions: ["Pork sausage from the oven", "Molten Chocolate Cake"],
    propositions: ["Iced Tea"],
  };
  ...
  const itemsProposed = menuData.filter((menuItem) =>
    menuItem.items.some((item) => response?.suggestions?.includes(item.name))
  );
                  ...
                  {itemsProposed
                    .filter((menu) =>
                      menu.items.filter((item) =>
                        dummyResponse.suggestions.includes(item.name)
                      )
                    )
                    .map((category) => (
                        ...
                        {category.items.length > 0 && (
                          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                              {category.items
                                .filter((food) =>
                                  dummyResponse.suggestions.includes(food.name)
                                )
                                .map((item) => (
